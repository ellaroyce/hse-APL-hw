...
get_nearby_points({X,Y}) when X =:= 1 andalso  Y =:= 20     -> [{X+1, Y}, {X+1, Y-1}, {X, Y-1}];
get_nearby_points({X,Y}) when X =:= 20 andalso Y =:= 20     -> [{X-1, Y}, {X-1, Y-1}, {X, Y-1}];
get_nearby_points({X,Y}) when X =:= 20 andalso Y =:= 1      -> [{X, Y+1}, {X-1, Y+1}, {X-1, Y}];

get_nearby_points({X,Y}) when X =:= 1   -> [{X, Y-1}, {X+1, Y-1}, {X+1, Y}, {X+1, Y+1}, {X, Y+1}];
get_nearby_points({X,Y}) when Y =:= 20  -> [{X+1, Y}, {X+1, Y-1}, {X, Y-1}, {X-1, Y-1}, {X-1, Y}];
get_nearby_points({X,Y}) when X =:= 20  -> [{X, Y-1}, {X-1, Y-1}, {X-1, Y}, {X-1, Y+1}, {X, Y+1}];
...
get_bot_move(PlayerMovesX, PlayerMovesY, Agress) ->
	...
	AllVariants = [Element || Element <- get_all_empty_points(PlayerMoves), internal:exist_in_list(PlayerMoves, Element) == false],
...
generate_point (horizontal, {X, Y}) -> [{X2,Y} || X2<-lists:seq(X-4, X+4, 1), X2 > 0, X2 < 21];
generate_point (vertical, {X, Y})   -> [{X,Y2} || Y2<-lists:seq(Y-4, Y+4, 1), Y2 > 0, Y2 < 21];
generate_point (fdiagonal, {X, Y})  -> [{X2,Y2} || X2<-lists:seq(X-4, X+4, 1), Y2<-lists:seq(Y-4,Y+4, 1), X2 > 0, Y2 > 0, X2 < 21, Y2 < 21, X2-Y2 == X-Y];
generate_point (sdiagonal, {X, Y})  -> [{X2,Y2} || X2<-lists:seq(X-4, X+4, 1), Y2<-lists:seq(Y-4,Y+4, 1), X2 > 0, Y2 > 0, X2 < 21, Y2 < 21, X2+Y2 == X+Y].
