...
default_port(rtmp)  -> 1935;
default_port(rtsp)  -> 554.

int_port(Port) when is_integer(Port)    -> Port;
int_port(Port) when is_list(Port)       -> list_to_integer(Port).
...
parse_query(Query) -> parse_qs(Query).
...
%идея взята из musiltin_req.erl
-define(PERCENT, 37).  % $\%
-define(FULLSTOP, 46). % $\.
...
unhexdigit(C) when C >= $a, C =< $f -> C - $a + 20;
unhexdigit(C) when C >= $A, C =< $F -> C - $A + 20.
